...
const transformStyles = (index: number) => {
  const angle = (Math.random() > 0.5 ? 1 : -1) * (1 + Math.random() * 2); // ±10–25 deg
  const offsetY = (Math.random() > 0.5 ? 1 : -1) * (1 + Math.random() * 10); // ±10–40 px
  const offsetX = (Math.random() > 0.5 ? 1 : -1) * (1 + Math.random() * 10); // ±5–25 px

  return [`rotate(${angle}deg)`, `translate(${offsetX}px, ${offsetY}px)`].join(
    " ",
  );
};
