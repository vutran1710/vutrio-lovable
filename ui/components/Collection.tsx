...
export const Collection = ({ item, index = 0 }: CollectionProps) => {
  ...
  useEffect(() => {
    const angle = (Math.random() > 0.5 ? 1 : -1) * (1 + Math.random()); // ±1–3 deg
    const offsetY = (Math.random() > 0.5 ? 1 : -1) * (1 + Math.random() * 5); // ±1–11 px
    const offsetX = (Math.random() > 0.5 ? 1 : -1) * (1 + Math.random() * 5); // ±1–11 px
  ...
  return (
    <div
      className={`collection-bg-${item.color} p-6 shadow-md`}
      style={{
